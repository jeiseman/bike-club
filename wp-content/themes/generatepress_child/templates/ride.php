...
if ($aid) {
    $mystatus =  get_post_meta($aid, 'ride_attendee_status', true);
    $wlnum = get_post_meta($aid, 'wait_list_number', true);
    if (empty($wlnum) || $wlnum == 0)
	    $is_waitlisted = false;
    else
	    $is_waitlisted = true;
    $license = get_post_meta($aid, 'car_license', true);
    $emergency_phone = get_post_meta($aid, 'emergency_phone', true);
    $cell_phone = get_post_meta($aid, 'cell_phone', true);
}
$wlas = "";
